      ...
      <div className="sticky top-0 z-50 bg-white dark:bg-black border-b border-black/10 dark:border-white/5 p-4 md:px-8 md:py-4 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 shadow-sm dark:shadow-none">
        ...
        <div className="flex flex-wrap items-center gap-2 sm:gap-3 w-full sm:w-auto">
          <select
            ...
            className="bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 text-sm text-slate-900 dark:text-white px-4 py-2 outline-none hover:border-[#07308D] transition-colors rounded-lg cursor-pointer"
          >
          ...
          <button
            onClick={handleSave}
            disabled={isSaving}
            className="flex items-center gap-2 bg-[#07308D] text-white px-5 py-2 text-sm font-bold uppercase tracking-wider hover:bg-[#07308D]/80 transition-colors rounded-lg shadow-lg shadow-[#07308D]/20 disabled:opacity-50 disabled:cursor-not-allowed"
          >
      ...
      <div className="max-w-7xl w-full mx-auto p-4 md:p-8 pb-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 flex flex-col gap-6 w-full max-w-full">
          <input
            ...
            className="w-full bg-transparent border-b border-black/10 dark:border-white/10 text-slate-900 dark:text-white text-2xl md:text-3xl lg:text-4xl font-serif font-bold py-3 outline-none placeholder-slate-400 dark:placeholder-white/20 focus:border-[#07308D] transition-colors"
          />
          <textarea
            ...
            rows={2}
            className="w-full bg-white dark:bg-white/5 border border-black/10 dark:border-white/10 text-slate-700 dark:text-gray-300 px-5 py-4 text-base outline-none focus:border-[#07308D] transition-colors rounded-lg resize-none font-serif placeholder-slate-400 dark:placeholder-white/20"
          />
                  ...
                  {CONTENT_TYPES.map(t => (
                    <button
                      ...
                      className={`py-2.5 text-[9px] font-bold rounded-lg uppercase tracking-wider border transition-all ${
                        form.type === t
                          ? 'bg-[#07308D] border-[#07308D] text-white shadow-lg shadow-[#07308D]/20'
              ...
              <div className="space-y-4">
                ...
                <div className="flex items-center justify-between p-4 bg-slate-50 dark:bg-white/5 border border-black/5 dark:border-white/10 rounded-xl h-[90px]">
                  ...
                  <button
                    ...
                    className={`relative w-12 h-6 rounded-full transition-colors ${form.showOnHomepage ? 'bg-[#07308D]' : 'bg-slate-300 dark:bg-slate-700'}`}
                  >
            ...
            <div className="space-y-4 pt-4 border-t border-black/5 dark:border-white/5">
              ...
              <div className="space-y-3">
                <input
                  ...
                  className="w-full bg-slate-50 dark:bg-black/40 border border-black/10 dark:border-white/10 text-slate-900 dark:text-white px-4 py-3 text-sm outline-none focus:border-[#07308D] transition-colors rounded-xl placeholder-slate-400 dark:placeholder-slate-700 font-medium"
                />

                <div className="group relative flex items-center bg-slate-50 dark:bg-black/40 border border-black/10 dark:border-white/10 rounded-xl focus-within:border-[#07308D] transition-all">
                  <input
                    type="text"
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full bg-slate-50 dark:bg-black/40 border border-black/10 dark:border-white/10 text-slate-900 dark:text-white px-4 py-3 text-sm outline-none focus:border-[#07308D] transition-colors rounded-xl placeholder-slate-400 dark:placeholder-slate-700 resize-none font-serif leading-relaxed"
                />
              </div>
            </div>
                ...
                {availableTags.map(tag => (
                  <button
                    ...
                    className={`text-[9px] px-3 py-1.5 rounded-lg border font-bold uppercase transition-all ${
                      tags.includes(tag)
                        ? 'bg-[#07308D]/20 border-[#07308D]/50 text-[#07308D]'
                        : 'bg-slate-50 dark:bg-white/5 border-black/5 dark:border-white/10 text-slate-500 hover:text-slate-900 dark:hover:text-white'
                    }`}
                  >
                    {tag}
